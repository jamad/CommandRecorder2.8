...
#==============================================================
bl_info = {
"name" : "CommandRecorder_jam",# プラグイン名
"description" : "find commandrecorder panel by viewport > n key",# プラグインの説明
"author" : "j",# 作者
...
class Muthird_UI(bpy.types.Panel):
    ...
    def draw_header(self, context):self.layout.label(text = "", icon_value = self.Header_Icon)
    #メニューの描画処理
    def draw(self, context): self.layout.label(text="") #文字列表示するだけ
...
try:
    ...
    print('debug A - done successfully')
...
class CR_List_Selector(UIList):
    def draw_item(self, context, layout, data, item, icon, active_data,active_propname, index):layout.label(text = item.name)
...
def Play(Commands) :
    print('play command is executed')
    scene = bpy.context.scene
...
def StrageFile() :
    ...
    AddonDirector = os.path.dirname(os.path.abspath(__file__))#アドオン管理システムの絶対パスを取得
    AddonDirector = AddonDirector.replace('2.80','2.81')# version update !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!  checking AddonDirector',AddonDirector)
...
def Load():
    ...
    file=StrageFile()
    print('ファイル',file)
    File = open(file , 'r')#読み込みモードでファイルを開くっす 
    List = []
    for Line in File:
...
class CR_PT_Instance(bpy.types.Panel):
    ...
    bl_region_type = 'UI'# メニューを表示するリージョン
    bl_category = "CommandRecorder"# メニュータブのヘッダー名
    bl_label = "Buttons of commands"# タイトル
...
def Num_Instance_Updater(self, context):
     items = []
     for Num_Loop in range(len(CR_Prop.Instance_Name)): items.append((str(Num_Loop) , "{0}".format(Num_Loop+1) , ""))
     return items
...
#-------------------------------------------------------------------------------------------
#使用されているクラスを格納
Class_List = [CR_OT_String,CR_Prop,CR_List_Selector,CR_OT_Selector,CR_List_Command,Command_OT_Play,Command_OT_Add,CR_OT_Command,
CR_List_Instance,CR_OT_Instance,CR_List_PT_VIEW_3D,CR_PT_Instance_VIEW_3D,CR_List_PT_IMAGE_EDITOR,CR_PT_Instance_IMAGE_EDITOR,]
